...
frappe.ui.form.on("EMP201 Submission", {
    refresh: function(frm) {
        ...
        if (frm.doc.name && frm.fields_dict.name_display_html) {
            // Styling for better readability and to ensure it uses available width.
            // Using a slightly smaller font, ensuring it wraps, and looks like a data display.
            let name_html = `
                <div style="
            ...
            // No need to call frm.refresh_field("name_display_html") after setting options for an HTML field,
            // the change should reflect. If not, it can be added back.
        }
                ...
                frm.call({
                    ...
                    callback: function(r) {
                        if (r.message) { // r.message is the updated doc from the server
                            frappe.model.sync(r.message); // Sync client doc with server version
                            frm.refresh(); // Refresh the entire form, which also handles dirty state
